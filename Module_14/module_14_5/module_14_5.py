...
async def sing_up(message):
    await message.answer('Введите имя пользователя(используйте только латинские буквы): ')
    await RegState.username.set()


@dp.message_handler(state=RegState.username)
async def set_username(message, state):
    if not user_exists(message.text):
...
@dp.message_handler(state=RegState.email)
